...
import HuangSe from "../../assets/center/ZhongXinCeShuJuHuiZong/北京.png";
import LvSe from "../../assets/center/ZhongXinCeShuJuHuiZong/绿色.png";
...
const images = ref([HuangSe, LvSe, HongSe])
const getImages = (i:number) => {
    return images.value[i % images.value.length]
}
...
function handleLink() {
    ...
    window.open(props.link+"?timenow="+timestampMs, "_blank")
}
...
<template>
    <div class="title_box">
        <div class="title_box_content">
            ...
            <span @click="handleLink" style="cursor: pointer;">中心侧数据汇总</span>
        </div>
    </div>
    <div class="table_box">
            ...
            <Vue3SeamlessScroll :visibleCount="5" :list="tableList" :step="1" :singleHeight="70" hover>
                <template v-slot="{ data,index }">
                    <div class="table_box_content_item">
                        ...
                        <div class="address">
                            <span :style="{
                                backgroundImage: `url(${getImages(index)})`
                            }">
                                {{ data.source }}
                            </span>
                        </div>
