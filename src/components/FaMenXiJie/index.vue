...
const Yuan = [HongSe, LvSe, HuangSe, LanSe, ZiSe]
const getYuan = (index: number) => {
    return Yuan[index % Yuan.length]
}


const { type = "", address = "", url = "", link = "" } = defineProps<{
    type: string,
    address: string,
    url: string,
    link: string
...
async function getData() {
    const res = await axios.get(url + "/api/manage/kongTiaoData", {
        headers: {
    ...
    if (res.status === 200) {
        if (Array.isArray(res.data.data.rows) && res.data.data.rows.length > 0 && res.data.data.total != total.value) {
...
<template>
    ...
    <div class="table_box">
        ...
        <div class="table_box_content">
            <!-- <Vue3SeamlessScroll :key="key" :list="tableList" :step="1" :direction="'up'" hover>
                <template v-slot="{ data,index }">
                    <div class="table_box_content_item">
                        <span>
                            <img style="margin-right: 2px;" :src="getYuan(index)" alt="" srcset="">
            ...
            <LoopScroll :dataSource="tableList" itemKey="nongDuDataId" direction="up">
                <template v-slot="{ item, index }">
                    <div class="table_box_content_item">
                        <span>
                            <el-tooltip :content="item?.deviceSn" placement="top">
                                <span>{{ item?.deviceSn }}</span>
                            </el-tooltip>
                        </span>
                        <span>
                            <el-tooltip :content="item?.fengLiangFanKui" placement="top">
                                <span>{{ item?.fengLiangFanKui }}</span>
                            </el-tooltip>
                        </span>
                        <span>
                            <el-tooltip :content="item?.yaLiSheDing" placement="top">
                                <span>{{ item?.yaLiSheDing }}</span>
                            </el-tooltip>
                        </span>
                        <span>
                            <el-tooltip :content="item?.faWeiFanKuan" placement="top">
                                <span>{{ item?.faWeiFanKuan }}</span>
                            </el-tooltip>
                        </span>
                    </div>
                </template>
